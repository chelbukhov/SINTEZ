...
contract CRYPTToken is StandardToken {
  ...
  constructor(address _CrowdsaleAddress) public {
    ...
    _;
     ...
     // Override
     function transfer(address _to, uint256 _value) public returns(bool){
          require(!lockTransfers, 'Transfers are temporarily prohibited');
...
contract Crowdsale is Ownable {
    ...
    function setPause() public onlyOwner { 
        //block all transfer tokens
        token.lockTransfer(true);
    }

    function setResume() public onlyOwner {
        //unblock transer tokens
    ...
    function saleTokens() internal {
        ...
        if (currentState == State.PreSale) {
            require (RPESALE_TOKEN_SUPPLY_LIMIT > soldTokens, 'HardCap of Pre-Sale is passed.'); //limit not yet aceessable
