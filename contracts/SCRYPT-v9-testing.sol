// текущая версия - 9 тестовая. уменьшены объемы!!!!, добавлены тестовые ф-ции (в конце кода)
...
//    uint public constant TOKEN_SOFT_CAP = 40000000 * 1 ether;
    ...
    function saleTokens() internal {
        ...
        if (currentState == State.PreSale) {
            require (RPESALE_TOKEN_SUPPLY_LIMIT > soldTokens, "HardCap of Pre-Sale is excedded."); 
            require (msg.value >= 20 ether, "Minimum 20 ether for transaction all Pre-Sale period");
        }
        if (currentState == State.PreICO) {
            require (RPEICO_TOKEN_SUPPLY_LIMIT > soldTokens, "HardCap of Pre-ICO is excedded.");
            if (now < preICOStartTime + 1 days){
                require (msg.value <= 20 ether, "Maximum is 20 ether for transaction in first day of Pre-ICO");
            }
        }
